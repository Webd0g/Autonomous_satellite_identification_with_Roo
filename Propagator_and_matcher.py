...
def satellite_propagator (telescope_position, image_time, image_ra_and_dec, sats_by_name, angular_distance_limit = 0.5):
    satellite_matches = {}

    #loads timescale object so that we can use .at() and .utc()
    ts = load.timescale()
    epoch = ts.utc(image_time.year, image_time.month, image_time.day, image_time.hour, image_time.minute, image_time.second)
    ...
    for name, satellite in sats_by_name.items():
        difference = satellite - telescope_position
        topocentric = difference.at(epoch)
        ra, dec, distance = topocentric.radec(epoch)
        ra_radians = ra.radians
        dec_radians = dec.radians
        

        angular_distance_from_TLE = np.arccos((np.sin(image_ra_and_dec[1])*np.sin(dec_radians)+np.cos(image_ra_and_dec[1])*np.cos(dec_radians)*np.cos(image_ra_and_dec[0]-ra_radians)))

        #Angular distance comparison, this is one of the large issues what do we choose for the threshold?
